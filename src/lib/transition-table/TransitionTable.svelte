<script lang="ts">
	...
	function generateUniqueLabelsMap(nodes: Node[]): Map<Node, string> {
		const taken = new SvelteSet<string>();
		const nodeToLabel = new SvelteMap<Node, string>();
		nodes.forEach((node) => {
	...
	<table id="transition-table" class="table table-zebra">
		<thead id="table-header">
			...
			<tr class="col-header">
				...
				{#each groupComputeInputs(table.inputs) as group, index (index)}
			...
			{#if PdaMode}
				...
				<tr class="col-header">
					...
					<td class="spacer v-borders h-borders"></td>
					{#each table.inputs as input, index (index)}
		...
		<tbody class="content">
			{#each table.content as row, rowIndex (rowIndex)}
						...
						<td class="cell h-borders v-borders">
							...
							{:else}
								<ul class="transition-output-list">
									{#each cell as output, outputIndex (outputIndex)}
