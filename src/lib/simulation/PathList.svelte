...
<script lang="ts">
	import { type PathLeaf, type FullPath, type ComputationTree } from './computationTree';
	...
	const {
		...
		onClick: (_leaf: PathLeaf) => void;
	} = $props();

	let containerHeight: number = $state(300);
	let scrollTop: number = $state(0);
	const totalHeight = $derived(leaves.length * ITEM_HEIGHT);
	...
	const visibleItems: { leaf: PathLeaf; path: FullPath }[] = $derived(
		leaves.slice(startIndex, endIndex).map((leaf) => ({ leaf, path: tree.getFullPath(leaf) }))
	);
	...
	function handleClick(leaf: PathLeaf, path: FullPath): void {
...
>
	<div style:height="{totalHeight}px">
		<ol class="absolute w-full" style:top="{startIndex * ITEM_HEIGHT}px">
			{#each visibleItems as { leaf, path }, i (startIndex + i)}
